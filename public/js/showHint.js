	...
	showHint: function(str){
		...
		if (str.length == 0) { // is the search string empty?
	    	...
	    	if (str !== "") { 
			    ...
			    var len=str.length;
			    employees.forEach(function(employee) { // loop through each employee
			    	...
			    	if(employee['fullname'].substr(0, len).toUpperCase() == str.toUpperCase()){
			    		...
			    		search_res.appendChild(obj.createBox(employee));
			    	}
			    });
			}
	    }
	...
	createBox: function(employee){
		...
		delForm.addEventListener("submit", function(event){
			...
			if (confirm("Are you sure you want to delete this employee?")) {
				...
				.then(obj.updateEmpArr(employee['id']))
				.catch(displayErrors)
	  		}else{
